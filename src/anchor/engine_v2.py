"""
Anchor Engine v2 — End-to-end v2 pipeline.

Bileşenler:
  - TopicExtractor (v1 — deterministic, değişmedi)
  - ScalableRuleStore (v2 — domain shard + bloom + semantic + binary index)
  - ConflictDetectorV2 (v2 — sentence-level claim extraction + edit distance)
  - PatchEngine (v2 — sentence-level override/patch/insert)
...
from anchor.rectify.v2.patch_engine import PatchEngine


class AnchorEngineV2:
    """
    Anchor v2 — LLM çıktıları için deterministik rectification engine.
    
    v1'den farkları:
      - ScalableRuleStore: Domain sharding + bloom + semantic
      - ConflictDetectorV2: Sentence-level claim extraction
      - PatchEngine: Sentence-level override/patch
      - Binary index: Cold start < 50ms
    
    Kullanım:
        engine = AnchorEngineV2(rules_path="rules/")
        engine.build()
    ...
    def __init__(self, rules_path: str, index_path: Optional[str] = None):
        ...
        self.detector = ConflictDetectorV2()
